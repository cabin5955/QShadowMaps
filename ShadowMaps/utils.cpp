...
Texture2D &qLoadTextureFromFile(const QString file, GLboolean alpha)
{
    // Create Texture object
    Texture2D *texture = new Texture2D();
    if (alpha)
    ...
    // Now generate texture
    QImage flipImg = img.mirrored(false,true);
    texture->Generate(flipImg.width(), flipImg.height(), flipImg.bits());
    return *texture;
}
